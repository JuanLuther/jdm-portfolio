  ...
  return (
    <header className="bg-base-100 sticky top-0 z-50 container mx-auto">
      <nav className="navbar">
        ...
        >
          <span
            className={`block w-6 h-0.5 bg-base-content transition-all duration-300 group-hover:bg-dark-teal 
      ${isMobileMenuOpen ? "rotate-45 translate-y-[5.7px]" : ""} 
    `}
          ></span>
          <span
            className={`block w-6 h-0.5 bg-base-content transition-all duration-300 my-[4px] group-hover:bg-dark-teal 
    ...
    `}
      ...
      >
        {navLinks.map((link) => (
          // <li> is just a container here, needs no DaisyUI tab classes
          <li key={link.href} role="presentation">
            <Link
              to={link.href}
              role="tab" // Correct role for the individual tab button/link
              className={`tab ${
                location.pathname === link.href
                  ? "tab-active text-primary border-primary"
                  : ""
        ...
        <li>
          <label className="swap swap-rotate text-center tab">
            {/* this hidden checkbox controls the state */}
            <input
              type="checkbox"
              className="theme-controller"
